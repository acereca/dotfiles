...
prompt_virtualenv() {
  ...
  if [[ -n $VIRTUAL_ENV && -n $VIRTUAL_ENV_DISABLE_PROMPT ]]; then
    prompt_segment blue black "(`basename $(echo $virtualenv_path | cut -d- -f1)`)"
  fi
}
...
prompt_status() {
  local symbols
  symbols=()
  [[ $RETVAL -ne 0 && $RETVAL -ne 148 ]] && symbols+="%{%F{white}%}`echo $RETVAL`"
...
prompt_bg() {
  [[ $(jobs -l | wc -l) -gt 0 ]] && prompt_segment yellow white "%{%F{white}%}`jobs -l | wc -l`"
}
